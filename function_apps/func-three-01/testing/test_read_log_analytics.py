...
@patch("project.read_log_analytics.get_max_times_log_for_each_storage")
...
@patch("project.read_log_analytics.get_workspace_table",Mock(return_value="data_for_type_query"))
...
@patch("project.read_log_analytics.time_index","days")
@patch("project.read_log_analytics.time_period_for_check_last_fetch","30")
@patch("project.read_log_analytics.workspace_id","workspace_id")
@patch("project.read_log_analytics.return_seconds",Mock(return_value=datetime.date(2023,12,12)))
@patch("project.read_log_analytics.timedelta",Mock(return_value="30"))
@patch("project.read_log_analytics.create_log_query_client")
...
@patch("project.read_log_analytics.return_seconds",Mock(return_value=datetime.date(2023,12,12)))
@patch("project.read_log_analytics.timedelta",Mock(return_value="30"))
@patch("project.read_log_analytics.create_log_query_client",Mock(return_value=log_query_client()))
...
@patch("project.read_log_analytics.DefaultAzureCredential",Mock(return_value="default azure credential"))
@patch("project.read_log_analytics.LogsQueryClient")
...
@patch('project.read_log_analytics.get_times_logs_per_table', Mock(side_effect = [[1,2],[3,4],[5,6]]))
...
@patch('project.read_log_analytics.groupby',Mock(return_value=[("storage_name",{"storage_name":"storage1","time":mock_time(2023,11,12)}),
...
@patch('project.read_log_analytics.next',Mock(side_effect=[{"storage_name":"storage1","time":mock_time(2023,11,12)},
